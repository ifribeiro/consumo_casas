...
def kl_divergence(original, samples, bins):
    p_original, _ = probs(original, bins=bins)
    divergences = []
    for s in samples:
        p, _ = probs(s, bins=bins)
...
def js_divergence(original, samples, col=0, bins=50):
    """
    ...
    """
    divergences = []
    p, _ = probs(original, bins=bins)
    for s in samples:
        q, _ = probs(s[:, col], bins)
        js = jensenshannon(p, q)
        divergences.append(js)
    return divergences
